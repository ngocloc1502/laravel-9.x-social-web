  ...
  <div class="row justify-content-center">
    <div class="col-md-10">
      ...
      <div class="card">
        ...
        <div class="card-body">
        <form method="POST" action="{{ route('news.update', ['id' => $news->id]) }}" aria-label="{{ __('Upload') }}" accept-charset="UTF-8" enctype="multipart/form-data">
          @csrf
          @method('PUT')
              ...
              <div class="col-md-9">
                <select class="custom-select form-control" name="category" id="category">
                  @foreach ($categories as $row)
                    <option value="{{ $row->id }}" {{ $row->id==$news->category_id?"selected":'' }}>{{ $row->name }}</option>  
                  @endforeach
                </select>
            ...
            <div class="form-group row">
              ...
              <div class="col-md-9">
                <textarea id="introduction" cols="10" rows="8" class="form-control{{ $errors->has('introduction') ? ' is-invalid' : '' }}" name="introduction" value="{{ old('introduction') }}"  autofocus>{{ $news->introduction }}</textarea>
            ...
            <div class="col-md-9">
              <textarea id="content" cols="10" rows="15" class="ckeditor form-control{{ $errors->has('content') ? ' is-invalid' : '' }}" name="content" value="{{ old('content') }}" required autofocus>{{ $news->content }}</textarea>
